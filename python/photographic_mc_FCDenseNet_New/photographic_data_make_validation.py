...
def make_data_from_distribution(C):
    ...
    for idx, track_number in enumerate(track_numbers):
        ...
        for i, bbox in enumerate(bboxes):
            ...
            input_photo = np.array(input_photo, dtype=np.float32)
            output_truth = np.array(output_truth, dtype=np.float32)

            np.save(input_file, input_photo)
            np.save(output_file, output_truth)

            x, y = input_photo, output_truth
            ...
            x = x.resize( [C.resolution*5,  C.resolution*5] )
            y = y.resize(  [C.resolution*5,  C.resolution*5] )
