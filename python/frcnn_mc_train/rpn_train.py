...
def rpn_train(C):
    ...
    LRCallback = tf.keras.callbacks.ReduceLROnPlateau(
        monitor='val_loss', factor=0.1, patience=20, verbose=0, mode='auto',
    ...
    posNumCallback = EarlyStoppingAtMinMetric('rpn_out_class_positive_number', 400)

    # compile the model
    model.compile(optimizer=adam, loss={'rpn_out_class' : rpn_class_loss,\
                                        'rpn_out_regress': rpn_regr_loss},\
                                    metrics={'rpn_out_class': [unmasked_accuracy, positive_number],\
    ...
    model.fit(x=inputs, y=[label_maps, delta_maps],\
                ...
                callbacks = [CsvCallback, LRCallback, posNumCallback])
