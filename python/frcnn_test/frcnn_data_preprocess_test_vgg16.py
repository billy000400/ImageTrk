...
def preprocess(C):
    ...
    bbox_Num = len(pd.read_csv(C.test_bbox_table_reference, index_col=0).index)
    bbox_idx = 0
    ...
    inputs_npy = np_dir.joinpath('inputs_test.npy')
    labels_npy = np_dir.joinpath('label_maps_test.npy')
    deltas_npy = np_dir.joinpath('delta_maps_test.npy')
